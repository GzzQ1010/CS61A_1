...
def merge(lst1, lst2):
    ...
    #     for a in range(i,len(new_list)):
    #         if new_list[i]>new_list[a]:
    #             help_element=new_list[i]
    #             new_list[i]=new_list[a]
    #             new_list[a]=help_element
    # return new_list
    
    "Recursive method:"## when given lists are sorted
    if lst1==[] or lst1==None:
        return lst2
    elif lst2==[] or lst2==None:
        return lst1
    elif lst1[0]<lst2[0]:
...
def count_palindromes(L):
    ...
    """
    if len(L)==1:
        if L[0].lower()==L[0][::-1].lower():
            return 1
    else:
        if L[0].lower()==L[0][::-1].lower():
            return 1+count_palindromes(L[1:])
        else:
            return count_palindromes(L[1:])
